...
final class GoogleDriveProvider: SyncProviderProtocol {
    private let provider: FolderSyncProvider

    let name = SyncProvider.googledrive

    init(storageRootProvider: @escaping () -> String = { StateManager.loadState().cloudSyncPath }) {
        self.provider = FolderSyncProvider(name: .googledrive, storageRootProvider: storageRootProvider)
    }
